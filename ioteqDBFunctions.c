...
const Tag_t* parseTag(char* strTest, char delim[], const Tag_t* currentTag) {
	...
	for (int i = 0; i <= currentTag->numOfChildren; i++) {
		...
		const Tag_t* tempTag = tree + i
				+ ((currentTag->childPtr / sizeof(Tag_t)));
...
uint8_t* iterateChildrenValues(const Tag_t* tag, uint8_t* dataArray,
		uint8_t index) {
...
uint8_t* getValue(const Tag_t* tag) {
	if (tag->numOfChildren != 0) {
		...
		if (currentChild->nextSibling != 0) {
			const Tag_t* nextSibling = tree
					+ (currentChild->nextSibling / sizeof(Tag_t));
		...
		if (tag->isPersistent) {
			...
			getPrivateTag(tag->persistentPtr, data + tag->valuePtr);
			return data + tag->valuePtr;
		}else {
			return data + tag->valuePtr;
		}
...
void setValue(const Tag_t* tag, uint8_t* value) {
	...
	if (tag->numOfChildren != 0) {
		...
		if (tag->isPersistent) {
			...
			setPrivateTag(tag->persistentPtr, value);
		} else {
